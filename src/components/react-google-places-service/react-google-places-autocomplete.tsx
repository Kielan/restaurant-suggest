...
export default class ReactGooglePlacesAutocomplete extends React.Component {
  static propTypes = {
  ...
  componentDidMount() {
    ...
    const handleAutoComplete = () => {
      this.autocomplete = new window.google.maps.places.Autocomplete(
        ...
        console.log('is this defined? ', this.autocomplete);
      ...
      console.log('is this defined x2? ', this.autocomplete);
    };
...
export class ReactCustomGoogleAutocomplete extends React.Component {
  ...
  onChange(e) {
    const { types = ['establishment', '(cities)'] } = this.props;

    if (e.target.value) {
      this.service.getPlacePredictions(
        { input: e.target.value, types },
        (predictions, status) => {
            console.log('kdl predictions updated ', predictions);
            console.log('kdl status updated ', status);
  ...
  componentDidMount() {
    if (this.props.input.value) {
      this.placeService = new window.google.maps.places.Autocomplete(this.refs.div);
  ...
  render() {
    return (
      <div>
        ...
        <div className="kdlgooglemaplist" ref="div" />
      </div>
    );
  }
}
